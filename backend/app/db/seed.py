...
def _parse_employee_file(path: Path) -> dict:
    ...
    email = require_field(fields, "Corporate email", path)
    return {
        "username": email.split("@")[0],
        "email": email,
        "full_name": parse_name(text, fallback=employee_id),
        "role": Role.EMPLOYEE,
...
def _parse_helpdesk_file(path: Path) -> dict:
    ...
    display_name = parse_name(text, fallback=helpdesk_id)
    username = display_name.lower().replace(" ", ".")
    escalation_authority_value = require_field(fields, "Escalation authority", path)
    return {
        "username": username,
