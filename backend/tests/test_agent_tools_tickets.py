...
async def test_record_task_handler_creates_task(db_session):
    # No cleanup_run() here: record_task_handler inserts a Task through
    # db_session whose classified_by_run_id FK-references this run, and
    # Postgres holds a FOR KEY SHARE lock on that run row for db_session's
    # entire (savepoint-based, not-yet-rolled-back) transaction -- see the
    # long comment on _make_run in test_tickets_service.py for the full
    # deadlock mechanics. Deleting the run here would block forever.
